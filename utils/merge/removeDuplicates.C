/*
Macro imported from MT2Analysis2015 - slightly modified to cope with nanomT2 output
...
Note: if you don't know whether there are duplicates, you should run on each baby-tree with the option 'false' as 2nd argument the first time. Only if duplicates will be found, run again with option 'true' to fill a new tree without duplicates. This will save time!
*/
...
void removeDuplicates(string inputFile="duplicates.root",
  ...
  unsigned int run,lumi;  // nanoAODs
  //int run,lumi;         // americans
  unsigned long long evt;
  oldtree->SetBranchAddress("run",&run);
  oldtree->SetBranchAddress("luminosityBlock",&lumi);
